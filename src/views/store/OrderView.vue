<template>
  <div>
    ...
    <el-scrollbar>
      ...
      <el-dialog v-model="centerDialogVisible" title="选择分站" width="45%" align-center>
        <el-form ref="ruleFormRef" :model="allocationList" label-width="120px" status-icon>
      ...
      <el-input v-model="allocationList.substationName" disabled />
          ...
          <el-form-item label="选择分站">
            <el-select v-model="allocationList.substationId" filterable placeholder="Select">
              <el-option v-for="item in substaion" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
      ...
      <!--  stripe   highlight-current-row    highlight-current-row   -->
      <!-- " -->
      <el-table :data="orderData" class="table" border :row-class-name="tableRowClassName">
        ...
        <el-table-column prop="status" label="状态"  width="100">
          <template #default="scope">
            <span v-if="scope.row.status == 0">等待调度</span>
            <span v-if="scope.row.status == 1">缺货中</span>
        ...
        <el-table-column prop="type" label="类型"  width="100">
          <template #default="scope">
            <span v-if="scope.row.type == 0">普通订单</span>
        ...
        <el-table-column label="操作" width="300">
          <template #default="scope">
            ...
            <el-button v-if="scope.row.status < 2" type="primary" style="margin-right: 16px"
